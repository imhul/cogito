...
class DataTable {
    ...
    getData() {
        ...
        this.el.querySelectorAll("thead th")
            .forEach(col => props.push(col.innerHTML));
        this.el.querySelectorAll("tbody > tr")
            .forEach(row => {
                let item = {};
                row.querySelectorAll("td")
                    .forEach((col, i) => item[props[i]] = col.innerHTML);
                res.push(item);
            });
        return res;
    }
    ...
    pagination() {
        ...
        document.querySelector(`#${this.el.id}_pagination`).appendChild(list);
    }
    ...
    init() {
        ...
        for (let i = 1; i <= 5; i++)
            ppChoose.innerHTML += `<option ${i == 1 ? 'selected' : ''}value='${i * 10}'>${i * 10}</option>`;
        ppChoose.onchange = () => {
...
document.querySelectorAll(DT_TRIGGER).forEach(el => new DataTable(el).init());